...
class SyncIssuesStrategy(JiraStrategy[int]):
    ...
    def execute(self, project_key: str = None) -> int:
        ...
        try:
            ...
            logger.error(f"Project with key '{project_key}' not found in the database. Sync projects first.")
            return 0
        ...
        while start_at >= 0:
            ...
            response = requests.get(
                f"{self.base_url}{self._ENDPOINT}", params=params, auth=(self.email, self.token), timeout=30
            )
            data = response.json()
    ...
    def _sync_issue(self, data: dict, project: Project) -> Issue:
        ...
        issue_obj, _created = Issue.objects.update_or_create(
            ...
            defaults={
                ...
                "details": (
                    fields.get("description", {}).get("content", [{}])[0].get("content", [{}])[0].get("text", "")
