...
def test_list_projects_general_success_200(authenticated_client):
    """
...
@pytest.mark.django_db
def test_list_projects_general_with_valid_parameter(authenticated_client):
    """
    Testa se o endpoint aceita e processa o parâmetro issues_breakdown_months válido
    """
    url = reverse("list_projects_general")

    response = authenticated_client.get(f"{url}?issues_breakdown_months=3")
...
@pytest.mark.django_db
def test_list_projects_general_with_invalid_parameter(authenticated_client):
    """
    Testa se o endpoint trata parâmetro issues_breakdown_months negativo
    """
    url = reverse("list_projects_general")

    response = authenticated_client.get(f"{url}?issues_breakdown_months=-1")
...
@pytest.mark.django_db
def test_list_projects_general_empty_database(authenticated_client):
    """
...
@pytest.mark.django_db
def test_list_projects_general_structure_validation(authenticated_client):
    """
